	...
	try {
		...
		if (products?.data) {
			yield put({
				...
				payload: params.payload.category === "Staples" ? products.data.listItemCategories : products.data.listItems,
			});
		} else {
			yield put({
...
function* getAddons(action) {
	const response = yield call(Products.getAddons, action.payload);
	if (response) {
		if (response.data && response.data.listItems && response.data.listItems.items) {
			yield put({
