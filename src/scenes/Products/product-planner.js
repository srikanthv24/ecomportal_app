...
const ProductPlanner = ({
  ...
  const getAddrTypeClassName = (session, addrType) => {
    return sessionAddress[session] === addrType
      ? "btn-dark"
      : "btn-outline-dark";
  };
  ...
  const handleAddress = (session, key, value) => {
    ...
    if (typeof sessionAddress[session] === "number") {
      ...
      newAddressesObj = {
        ...addresses,
        [sessionAddress[session]]: sessionAddrCopy,
      };
  ...
  const getFieldValue = (session, fieldName) => {
    return typeof sessionAddress[session] === "number"
      ? addresses.other[sessionAddress[session]][fieldName]
      : addresses[sessionAddress[session]][fieldName];
  };

  const handleAddrTagClick = (session, addrType) => {
    if (
      typeof addrType === "number" &&
      typeof addresses.other[addrType] !== "object"
    ) {
  ...
  useEffect(() => {
    updateAddresses(
      Object.entries(sessionAddress).map(([key, value]) => ({
        ...(typeof value === "number"
          ? addresses.other[value]
          : addresses[value]),
        tag: typeof value === "number" ? "other" : value,
        postalcode: pincodes[sessionsOrder.indexOf(key)],
      }))
    );
  ...
  useEffect(() => {
    if (
      ...
      postalCodes?.listPostalCodes?.items?.length
    ) {
      setServicePinCodes(
        postalCodes?.listPostalCodes.items.map(
          (postalCodeObj) => postalCodeObj.postalcode
        )
      );
    }
  }, [postalCodes?.listPostalCodes]);
  ...
  useEffect(() => {
    if (pincodes && pincodes[selectedIndex].length === 6) {
      const filter = postalCodes?.listPostalCodes?.items.find(
        (pincode) => parseInt(pincodes[selectedIndex]) === pincode.postalcode
      );
      if (filter) {
  ...
  return (
    <div>
      ...
      <Accordion
        defaultActiveKey="0"
        className="mt-4 prd-planner-acc"
        style={{ zIndex: 2 }}
      >
        {/* BreakFast */}
                      ...
                      <>
                        ...
                        {pincodes[index].length !== 6 && (
                          <div className="card text-dark text-center bg-transparent border-0">
                            Enter your Pincode here to validate our services
                            availability
                          </div>
                        )}
                        {pincodes[index].length === 6 &&
                          (!servicePinCodes.find(
                            ...
                            <>
                              <div className="d-flex mb-2 mt-4">
                                <button
                                  ...
                                  type="button"
                                  class={`btn me-2 ${getAddrTypeClassName(
                                    sessionsOrder[index],
                                    "home"
                                  )}`}
                                >
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <FormControl
                              ...
                              onChange={(e) => {
                                ...
                                if (q != null) {
                                  updatedQuantity(q, idxx, deliver.id, index);
                                }
                              }}
                            />
                          </InputGroup>
                          <span
                            ...
                            onClick={() => {
                              console.log("index clicked", idxx, AddOnView);
                              //   console.log("remove clicked",AddOnView)
                              let newaddons = AddOnView[deliver.id].splice(
                                idxx,
                                1
                              );
                              let newaddons2 = AddOnView[deliver.id].filter(
                                (item) => item !== newaddons
                              );
                              ...
                              setAddOnView({
                                ...AddOnView,
                                [deliver.id]: newaddons2,
                              });
                              setValue(
                                `subscription[${index}].addon_items`,
                                newaddons2
                              );
                            }}
                          >
